...
Trang chủ (Hero)
- Tiêu đề: "Đặt lịch số thông minh".
- Hai CTA: "Khám phá Creator" và "Trở thành Creator".

Khám phá Creator
- Danh sách dạng lưới các Creator.
- Thẻ (card) gồm: ảnh đại diện, tên + lĩnh vực (tags), điểm đánh giá, nút "Xem lịch".

Trang cá nhân Creator
- Thông tin: bio ngắn, liên hệ, liên kết mạng xã hội (X/Twitter, Instagram,...).
- Lịch công khai: chế độ tuần/tháng, có bộ lọc múi giờ (timezone) để hiển thị giờ địa phương của người xem.

Lịch & Slot
- Calendar hiển thị các khoảng thời gian/slot do creator cấu hình (trạng thái: Open/Reserved/Locked/Completed).
- Bộ lọc: chế độ (Stable/Bidding), múi giờ, khoảng ngày.

Slot Stable (mua ngay)
- Thẻ slot: [giờ] — [giá] — nút "Đặt ngay".
- Hành động: gọi `stable_reserve(slot, amount)` và quy trình T0/T1 sau đó (xem phần Program Overview).

Slot Bidding (đấu giá)
- Thẻ slot: [giờ] — [giá khởi điểm] — đồng hồ đếm ngược — nút "Tham gia đấu giá".
- Hành động: `bid_place(slot, amount)`; hiển thị giá hiện tại, bước tăng tối thiểu, gia hạn chống snipe.

Định tuyến gợi ý
- `/` (Trang chủ)
- `/creators` (Khám phá)
- `/creator/[pubkey]` (Trang cá nhân + lịch công khai)
- `/slot/[slotPubkey]` (Chi tiết slot, tham gia/đặt ngay, theo dõi đếm ngược)

Thành phần giao diện (components)
- `Hero`, `CreatorCard`, `Calendar` (week/month), `SlotCard` (Stable/Bidding), `TimezoneSelector`, `Countdown`.

Dữ liệu & tích hợp Anchor
- Đọc `Platform`, `CreatorProfile`, `TimeSlot`, `BidBook` để render creator và slot.
- JS SDK: `@coral-xyz/anchor@^0.31.1` (apps/actions đã khai báo), dùng IDL của `timemarket` để truy vấn tài khoản và gọi ix.

Ghi chú UX
- Đồng hồ đếm ngược dựa trên `auction_end_ts` (hoặc mốc T0/T1), cập nhật mỗi giây; đồng bộ thời gian theo `Clock` on-chain khi cần độ chính xác.
- Bộ lọc múi giờ: lưu vào `localStorage`, mặc định dùng múi giờ hệ thống của người dùng.
